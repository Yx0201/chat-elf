...
/**
 * 06 交互语言(Ardot 设计稿 3:318,说明页)· 2026-08-31 移至 /language。
 *
 * 「场景 → 球体状态 → 鼠标/转场」映射表 + 三张硬规则卡 + H5 适配备注。
 * 本页无球锚点(球常驻层淡出),纯文档排版。入口:登录页底部链接。
 */

import Link from "next/link";

const ROWS: Array<{ scene: string; state: string; note: string }> = [
  { scene: "登录 / 孵化", state: "egg", note: "目光跟随光标;身体轻微摇。登录成功 burst;孵化页点「唤醒它」:egg→burst→idle。失败切 alert。" },
  { scene: "孵化 · 性格定格", state: "hexagon", note: "进入 swirl 转场;拖滑块时头转向该行。确认孵化 swirl 一圈后 morph 回 idle。人格与音色一次性确定,不可修改。" },
  ...
  { scene: "历史记录", state: "comet", note: "滚动时彗星尾巴相位跟着走;点进会话 orbit 转场回 wide。删除走两次确认。" },
  ...
  return (
    <div className="mimic-page min-h-dvh bg-white px-4 py-8 lg:px-14 lg:py-14">
      <div className="mx-auto flex w-full max-w-[1328px] flex-col gap-6 lg:gap-7">
        <Link href="/" className="text-[13px] text-[#787671] hover:text-[#A4A097]">
          ← 登录
        </Link>
        ...
        <div className="flex flex-col gap-2 rounded-xl bg-[#F9E79F] p-6">
          ...
          <p className="text-sm leading-[1.55] text-[#37352E]">
            窄屏不是等比缩小。登录/孵化改成球在上、表单在下;对话底栏保留陪伴/字幕切换、
            麦克风与结束;记忆/历史从设置抽屉进。球在 H5 不跟随手指(抬指会冻住目光),
            只保留点击彩虹。触区一律 ≥44px。
          </p>
        </div>
      </div>
